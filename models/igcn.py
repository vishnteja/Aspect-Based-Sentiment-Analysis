...
class IGCN(nn.Module):
  ...
  def forward(self, inputs):
    ...
    aspect2context, context_gate_scores = gate(
        context, context_gate, option=self.opt.gating)
    ...
    context2aspect, aspect_gate_scores = gate(
        aspect, aspect_gate, option=self.opt.gating)
    ...
    if self.opt.eval:
      return out, context_gate_scores, aspect_gate_scores
    return out
...
# Only Context and Aspect without gate
...
# Only Context information
class IGCN_5(nn.Module):

  def __init__(self, embedding_matrix, pos_matrix, opt):
    super(IGCN_5, self).__init__()
    self.opt = opt
  ...
  def forward(self, inputs):
    ...
    # Maxpool
    final_context = torch.max_pool1d(
        context.squeeze(3), kernel_size=context.shape[2]).squeeze(2)

    # FeatureVec
    final = self.dropout(final_context)
    out = self.dense(final)
    return out